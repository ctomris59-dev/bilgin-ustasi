...
export default function HeroHub({profile,onChangeAvatar,onOpenLessons,onOpenShop}){
  ...
  return <section className="v50-hero-page">
    <aside className="v50-status-panel v47-panel">
      ...
      <div className="v50-active-thumb"><StyleRender style={active}/></div>
    ...
    <div className="v47-stage-panel v50-stage-panel"><div className="v47-scene-glow"/>{isPreviewing&&<div className="v494-tryon-badge">DENEME · {preview.shortLabel}</div>}<div className={`v47-full-hero motion-${motion}`}><StyleRender className="v50-style-hero" style={preview} label={false}/></div><div className="v47-motion-stack">{MOTIONS.map(([id,label,icon])=><button key={id} className={motion===id?"is-active":""} onClick={()=>{setMotion(id);playPop();}}><span>{icon}</span><b>{label}</b></button>)}<button onClick={randomize}><span>🎲</span><b>Rastgele</b></button></div></div>
    ...
    <div className="v50-style-panel v47-panel">
      ...
      <div className="v50-style-grid">{pageStyles.map((style)=>{const owned=isStyleUnlocked(style.id,profile.xp||0);const isActive=active.id===style.id;const selected=preview.id===style.id;return <button key={style.id} className={`v50-style-card ${selected?"is-selected":""} ${isActive?"is-active":""} ${!owned?"is-locked":""}`} onClick={()=>previewStyle(style)}><div><StyleRender style={style} label={false}/>{!owned&&<i>🔒</i>}{isActive&&<em>✓</em>}</div><strong>{style.shortLabel}</strong><small>{owned?isActive?"TAKILI":"AÇIK":`${style.unlockXp} XP`}</small></button>;})}</div>
