...
const getPresetImage = (preset) => preset === "explorer" ? heroMaster : (CHARACTER_PRESETS[preset] || heroMaster);
...
export default function HeroHub({ profile, onChangeAvatar, onOpenLessons, onBuyItem }) {
  ...
  const heroSrc = getPresetImage(outfitPreset);

  function choose(item){setSelectedId(item.id);playPop();if(!owned.has(item.id)){onBuyItem?.(item);return;}onChangeAvatar?.({...profile.avatar,[item.slot]:item.id});}
  function choosePreset(outfitId){const item=ITEMS.find((entry)=>entry.id===outfitId);if(!item)return;setSlot("outfit");setSelectedId(outfitId);playPop();if(owned.has(outfitId))onChangeAvatar?.({...profile.avatar,outfit:outfitId});else onBuyItem?.(item);}
  function randomize(){const unlockedOutfits=ITEMS.filter((item)=>item.slot==="outfit"&&owned.has(item.id));const next=unlockedOutfits[Math.floor(Math.random()*unlockedOutfits.length)];if(next)choosePreset(next.id);}
  ...
  return <section className="v47-hero-page">
    ...
    <div className="v47-stage-panel"><div className="v47-scene-glow"/><div className={`v47-full-hero motion-${motion}`}><img src={heroSrc} alt="Bilgin Kaşif"/></div><div className="v47-motion-stack">{MOTIONS.map(([id,label,icon])=><button key={id} className={motion===id?"is-active":""} onClick={()=>{setMotion(id);playPop();}}><span>{icon}</span><b>{label}</b></button>)}<button onClick={randomize}><span>🎲</span><b>Rastgele</b></button></div></div>
    <div className="v47-wardrobe-panel v47-panel"><div className="v47-slot-tabs">{SLOT_TABS.map(([id,label,icon])=><button key={id} className={slot===id?"is-active":""} onClick={()=>{setSlot(id);setSelectedId(profile.avatar?.[id]||null);playPop();}}><span>{icon}</span>{label}</button>)}</div>
      ...
      <div className="v47-combos"><h3>Kombinasyon Önerileri</h3><div>{PRESET_LABELS.map(([id,label,preset])=><button key={id} className={profile.avatar?.outfit===id?"is-active":""} onClick={()=>choosePreset(id)}><img src={getPresetImage(preset)} alt=""/><span>{label}</span>{profile.avatar?.outfit===id?<i>✓</i>:<i>★</i>}</button>)}</div></div>
    </div>
