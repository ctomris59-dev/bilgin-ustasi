...
export default function ShopHub({profile,onChangeAvatar,onOpenHero,onOpenLessons}){
  ...
  function action(){
    playPop();
    if(isStyleUnlocked(selected.id,profile.xp||0)){
      if(selected.id!==activeId)onChangeAvatar?.({styleId:selected.id,characterStyle:"bilgin-kasif-master"});
      onOpenHero?.();
    }else onOpenLessons?.();
  }
  return <section className="v50-shop-page">
    ...
    <div className="v50-shop-shell v47-panel">
      ...
      <aside className="v50-shop-detail"><div className="v50-shop-detail-art"><StyleRender style={selected} label={false}/></div><div className="v50-shop-detail-copy"><span>STİL {String(selected.index).padStart(2,"0")}</span><h2>{selected.label}</h2><p>{selected.description}</p><ul><li>✦ Aynı Bilgin Kaşif karakteri</li><li>▣ Sabit 1024×1536 master çözünürlük</li><li>🏆 XP kazanarak kalıcı açılır</li></ul></div><button onClick={action}>{isStyleUnlocked(selected.id,profile.xp||0)?selected.id===activeId?"✓ ŞU AN TAKILI":"BU STİLİ KUŞAN →":`DERSLERLE ${Math.max(0,selected.unlockXp-(profile.xp||0))} XP KAZAN →`}</button></aside>
