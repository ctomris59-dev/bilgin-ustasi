...
import StyleRender from "../v50/StyleRender";
import { playPop } from "../../lib/sound";

const PAGE_SIZE=10;
export default function ShopHub({profile,onChangeAvatar,onOpenHero,onOpenLessons}){
  ...
  function action(){playPop();if(isStyleUnlocked(selected.id,profile.xp||0)){if(selected.id!==activeId)onChangeAvatar?.({styleId:selected.id,characterStyle:"bilgin-kasif-master"});onOpenHero?.();}else onOpenLessons?.();}
  return <section className="v50-shop-page">
    ...
    <div className="v50-shop-shell v47-panel">
      <div className="v50-shop-grid">{pageStyles.map((style)=>{const owned=isStyleUnlocked(style.id,profile.xp||0);const active=style.id===activeId;const selectedNow=style.id===selectedId;return <button key={style.id} className={`v50-store-card ${selectedNow?"is-selected":""} ${active?"is-active":""} ${!owned?"is-locked":""}`} onClick={()=>select(style)}><div><StyleRender style={style} label={false}/>{!owned&&<i>🔒</i>}{active&&<em>✓</em>}</div><span>STİL {String(style.index).padStart(2,"0")}</span><strong>{style.shortLabel}</strong><small>{owned?active?"TAKILI":"AÇIK":`${style.unlockXp} XP`}</small></button>;})}</div>
      <aside className="v50-shop-detail"><div className="v50-shop-detail-art"><StyleRender style={selected} label={false}/></div><div className="v50-shop-detail-copy"><span>STİL {String(selected.index).padStart(2,"0")}</span><h2>{selected.label}</h2><p>{selected.description}</p><ul><li>✦ Tek Bilgin Kaşif karakteri, 20 tam görünüm</li><li>▣ Her stil aynı 640×960 çözünürlük ve 2:3 oran</li><li>🏆 XP kazanarak kalıcı açılır</li></ul></div><button onClick={action}>{isStyleUnlocked(selected.id,profile.xp||0)?selected.id===activeId?"✓ ŞU AN TAKILI":"BU STİLİ KUŞAN →":`DERSLERLE ${Math.max(0,selected.unlockXp-(profile.xp||0))} XP KAZAN →`}</button></aside>
