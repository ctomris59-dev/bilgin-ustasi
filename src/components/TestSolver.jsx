...
export default function TestSolver({ test, isRetryTest = false, onFinish, onCancel }) {
  const [phase, setPhase] = useState("intro"); // intro | quiz | bonus | done
  const [timedMode, setTimedMode] = useState(false);
  const [index, setIndex] = useState(0);
  const [selected, setSelected] = useState(null);
  const [answers, setAnswers] = useState([]); // { qId, correct, chosenIndex }
