...
export default function ResultScreen({ result, xpEarned = 0, coinsEarned = 0, gemsEarned = 0, speedBonus = 0, newBadges = [], newLegendaryItems = [], newSticker, boostActive, onClose }) {
  ...
  return <div className="app-shell relative min-h-[82vh] overflow-hidden pb-10 pt-4">
    ...
    <div className="relative z-10 mx-auto max-w-xl">
      <header className="mb-5 text-center"><div className="relative mx-auto flex h-20 w-20 items-center justify-center"><div className={`absolute inset-0 rounded-full ${fullScore ? "animate-pulse-glow" : ""}`} style={{ background: `radial-gradient(circle,${fullScore?'rgba(255,209,102,.18)':'rgba(139,108,255,.17)'},transparent 68%)` }} /><div className="animate-level-ring relative flex h-16 w-16 items-center justify-center rounded-2xl border text-2xl font-black" style={{ color:fullScore?'#FFD166':'#A98CFF', background:fullScore?'rgba(255,209,102,.11)':'rgba(139,108,255,.12)', borderColor:fullScore?'rgba(255,209,102,.3)':'rgba(169,140,255,.28)' }}>{fullScore?'✦':'✓'}</div></div><p className="mt-3 text-[9px] font-black uppercase tracking-[.24em]" style={{ color:fullScore?'#FFD166':'#A98CFF' }}>{isRetryTest?'Rövanş Tamamlandı':'Görev Tamamlandı'}</p><h1 className="font-display mt-1 text-3xl font-black">{performance.title}</h1><p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-[#8793B4]">{performance.subtitle}</p></header>
