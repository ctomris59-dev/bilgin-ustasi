...
function ShopInspector({ profile, item, onBuyItem, onEquipItem, onOpenCharacter }) {
  ...
  return <aside className="v4-right-rail v4x-right-rail">
    <section className="v4x-rail-card v4x-inspector-card">
      ...
      {meta? <><div className="v4x-inspector-art rarity-${meta.rarity}"><img src={meta.cardAsset} alt={meta.label}/></div><h3>{meta.label}</h3><p>{meta.world.title} koleksiyonuna ait {meta.slotMeta.label.toLocaleLowerCase("tr-TR")}. Öğrenme görevlerinden kazandığın ödüllerle açılır.</p><div className="v4x-inspector-stats"><Stat value={`Sv. ${meta.world.unlockLevel}`} label="Bölge seviyesi" color={meta.world.accent}/><Stat value={meta.price||"Özel"} label="Coin fiyatı" color="#FFD166"/></div>{owned?<button className={`v4x-rail-primary ${equipped?"owned":"cyan"}`} onClick={()=>onEquipItem?.(meta)}>{equipped?"✓ Takılı · Çıkar/Koru":"Kuşan / Kullan"}</button>:meta.legendary?<div className="v4x-legendary-note">✦ Bu item satın alınmaz; ilgili testte kusursuz görev ile açılır.</div>:level<meta.world.unlockLevel?<div className="v4x-locked-note">Seviye {meta.world.unlockLevel}'e ulaşınca satın alınabilir.</div>:<button className="v4x-rail-primary gold" disabled={(profile.coins||0)<meta.price} onClick={()=>onBuyItem?.(meta)}>◈ {meta.price} · {(profile.coins||0)>=meta.price?"Satın Al":`${meta.price-(profile.coins||0)} coin daha`}</button>}<button className="v4x-rail-secondary" onClick={onOpenCharacter}>Karakter Envanterini Aç</button></>:<div className="v4x-inspector-empty">Dükkandan bir karta dokun; item burada büyük önizleme ile açılır.</div>}
    </section>
