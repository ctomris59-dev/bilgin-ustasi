...
export default function AnimatedAvatar({
  ...
  return (
    <div
      ...
      data-avatar-version="5.1.2-premium"
    ...
    >
      ...
      <svg className="v5p-rig-svg" viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        ...
        {showEquipment && loadout.back && <TintedMasterLayer uid={uid} slot="back" item={loadout.back} />}
      ...
      {showBadges && (
        <div className="v5p-status"><span>PREMIUM HERO</span><b>{equippedCount}/5</b></div>
      )}
    </div>
  );
}
...
function OutfitDetails({ item, uid }) {
  ...
  if (/crystal|galaxy|cosmic/i.test(id)) {
    ...
    return <g clipPath={`url(#outfit-${uid})`}><circle cx="143" cy="136" r="2" fill="#EAFBFF"/><circle cx="179" cy="161" r="1.7" fill="#FFD9FF"/><path d="M188 131 l2.5 5 5 2.5-5 2.5-2.5 5-2.5-5-5-2.5 5-2.5z" fill={accent}/></g>;
  }
  return null;
}
