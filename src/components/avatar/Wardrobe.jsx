...
export default function Wardrobe({ profile, onChangeAvatar, onChangePet, onChangeRoomSlot, onSelectItem }) {
  ...
  function choosePet(item) { playPop(); onSelectItem?.(item); onChangePet({ ...profile.pet, activeSpecies: profile.pet?.activeSpecies === item.id ? null : item.id }); }
  function choosePetAccessory(item) { playPop(); onSelectItem?.(item); onChangePet({ ...profile.pet, accessory: profile.pet?.accessory === item.id ? null : item.id }); }

  return (
    <div className="v4x-character-screen v43-character-screen">
      <section className="v4x-character-head v43-character-head">
        <div><span className="v4x-eyebrow">KAŞİF PROFİLİ</span><h2>Karakter & Envanter</h2><p>Derslerden kazandığın ekipmanları kuşan; karakterin gerçekten üzerindeki parçalarla değişsin.</p></div>
        ...
        <div className="v43-character-content">
          {tab === "equipment" && (
            <section className="v4x-inventory-panel v43-inventory-panel">
              <div className="v43-panel-title"><div><small>EKİPMAN KOLEKSİYONU</small><strong>Takmak istediğin parçayı seç</strong></div><span>{equipment.length} açık</span></div>
          ...
          {tab === "appearance" && (
            <section className="v4x-style-manager v43-style-manager">
              <div className="v4x-section-title"><div><small>KARAKTER STİLİ</small><strong>Temel karakter görünümü</strong></div><span>Ekipman katmanları bunun üzerine takılır</span></div>
              <div className="v4x-style-grid">
                {CHARACTER_STYLES.map((style) => { const active = (profile.avatar?.characterStyle || "auto") === style.id; return <button key={style.id} className={active ? "is-active" : ""} onClick={() => onChangeAvatar({ ...profile.avatar, characterStyle: style.id })}><div><img src={getCharacterStyleAsset(style.id)} alt="" /></div><strong>{style.label}</strong><small>{style.description}</small>{active && <b>✓ Aktif</b>}</button>; })}
              </div>
            </section>
          )}
        </div>
...
function CharacterStage({ profile, level }) {
  return <aside className="v43-character-stage-panel">
    <div className="v43-stage-kicker"><span>AKTİF KAŞİF</span><b>Lv. {level}</b></div>
    <div className="v43-character-stage">
      <RoomBackground room={profile.rooms?.bedroom} compact>
        <AvatarCanvas avatar={profile.avatar} size={330} showEquipment showBadges={false} />
    ...
    <div className="v43-learning-loop"><strong>Ders → Ödül → Ekipman</strong><small>Yeni item açmak için öğrenme görevlerini tamamla.</small></div>
  </aside>;
}
...
function countEquipped(profile) { return [profile.avatar?.outfit, profile.avatar?.shoes, profile.avatar?.headwear, profile.avatar?.face, profile.avatar?.back, profile.pet?.activeSpecies, profile.pet?.accessory].filter(Boolean).length; }
