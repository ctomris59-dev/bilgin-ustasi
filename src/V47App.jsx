...
export default function V47App() {
  ...
  function finishTest(result){
    ...
    const next={...profile,xp:(profile.xp||0)+rewards.xp,coins:(profile.coins||0)+rewards.coins,gems:(profile.gems||0)+gemsEarned,
      history:[...(profile.history||[]),{id:`h${Date.now()}`,testId:result.testId,subject:result.subject,date:new Date().toISOString(),correctCount:result.correctCount,totalCount:result.totalCount,xpEarned:rewards.xp,coinsEarned:rewards.coins,isRetry:false}],
      mistakeBox:[...(profile.mistakeBox||[]),...(result.wrongQuestions||[]).map((q)=>({id:`m${Date.now()}-${q.id}`,testId:result.testId,subject:result.subject,question:q,addedAt:new Date().toISOString(),resolved:false}))],
      stats:{...(profile.stats||{}),testsBySubject:{...(profile.stats?.testsBySubject||{}),[result.subject]:(profile.stats?.testsBySubject?.[result.subject]||0)+1},fullScoreBySubject:{...(profile.stats?.fullScoreBySubject||{}),...(result.correctCount===result.totalCount?{[result.subject]:(profile.stats?.fullScoreBySubject?.[result.subject]||0)+1}:{})}}};
    next.streak=updateStreak(next,new Date().toISOString());persist(next);setActiveTest(null);setPendingResult({result:{...result,fullScore:result.correctCount===result.totalCount},xpEarned:rewards.xp,coinsEarned:rewards.coins,speedBonus:0,newBadges:[],newLegendaryItems:[],newSticker:null,boostActive:false,gemsEarned});
  }

  function changeAvatar(avatar){persist({...profile,avatar});}
  function changePet(pet){persist({...profile,pet});}
  function buyItem(item){if(!item||item.legendary||(profile.unlockedItems||[]).includes(item.id))return;const requiredLevel=getWorldById(item.world)?.unlockLevel||1;if(getLevelInfo(profile.xp||0).current.level<requiredLevel){notify(`Bu eşya için Seviye ${requiredLevel} gerekiyor.`);return;}if((profile.coins||0)<(item.price||0)){notify("Yeterli coin yok.");return;}playCoin();persist({...profile,coins:profile.coins-item.price,unlockedItems:[...(profile.unlockedItems||[]),item.id]});notify(`${item.label} açıldı!`);}
  ...
  return <><AppShell profile={profile} tests={tests} syncStatus={syncStatus} activeSection={activeSection} tab={tab} onChangeTab={setTab} focusMode={focusMode}>
    ...
    :tab==="pets"?<CompanionHub profile={profile} onChangePet={changePet}/>
    :tab==="base"?<BaseHub profile={profile}/>
    :tab==="leaderboard"?<LeaderboardHub profile={profile}/>
    :<HeroHub profile={profile} onChangeAvatar={changeAvatar} onOpenLessons={()=>setTab("lessons")} onBuyItem={buyItem}/>} 
  </AppShell>{toast&&<div className="v47-toast">{toast}</div>}</>;
}
